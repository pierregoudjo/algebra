  ...
  fc.assert(
    fc.property(
      ...
      (x, y, z) => {
        ...
        //Right inverse
        expect(Math.round(multiply(x, multiplicationInvert(x)))).toEqual(multiplicationEmpty())

        //Left Inverse
        expect(Math.round(multiply(multiplicationInvert(x), x))).toEqual(multiplicationEmpty())
      },
    ),
  );
});
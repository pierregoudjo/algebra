...
export const lt = <T>(o: OrdFn<T>): BinaryRelation<T> =>
  (x, y) => o(x, y) && o(y, x) ? false : o(x, y);
export const gte = <T>(o: OrdFn<T>): BinaryRelation<T> =>
  (x, y) => !lt(o)(x, y);
export const lte = <T>(o: OrdFn<T>): BinaryRelation<T> => (x, y) => o(x, y);
